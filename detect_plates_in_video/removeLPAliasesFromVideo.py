...
while True:
  ...
  for imagePath in sorted(myPaths):
    print("[INFO] reading video file {}...".format(imagePath))
    ...
    while vs is None:
      vs = cv2.VideoCapture(imagePath)
      if vs is None:
        print("video: {} is empty".format(imagePath))
        time.sleep(1)
    ...
    while True:
      ...
      if (ret==False):
        ...
        if (conf["move_video_file"] == "true"):
          try:
            # os.rename(imagePath, outputPathSaveOriginalImage) #does not work between two different file systems
            shutil.move(imagePath, outputPathSaveOriginalImage)
          except OSError as e:
